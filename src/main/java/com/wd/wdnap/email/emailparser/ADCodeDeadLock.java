...
@Component
public class ADCodeDeadLock implements WDNAPEmailParser {
	private static final Logger LOGGER = LoggerFactory
			.getLogger(ADCodeDeadLock.class);
	@Override
			...
			for (int i = ele.size() - 1; i > 0; i--) {
				...
				if (value.contains("Event Type")) {
					...
					if (ele.get(i + 1) != null && ele.get(i + 1).getElementsByTag("tr").size() >= 3) {
						...
						String innermsg = ADCodeDeadLock.parseInnerText(errorMessage, cnapMailNotificationObj);
	...
	private static String parseInnerText(String innerText, WDNAPMailNotificationObject cnapMailNotificationObj) {
		...
		if (StringUtils.isNoneEmpty(msg)) {
			...
			if(msg.contains("JVM deadlock detected"))
				cnapMailNotificationObj.setSeverity("Error");
			if(msg.contains("JVMwas re-started"))
				cnapMailNotificationObj.setSeverity("Info");
			...
			if (msg.contains("1)") || msg.contains("condition 1") || msg.contains("JVM|Memory:")) {
				...
				if (balanceString.contains("threshold")) {
					...
					if(StringUtils.isNotBlank(callsPerMin))
						cnapMailNotificationObj.setCallsPerMin(callsPerMin.trim());
					
					String errPerMin = StringUtils.substringBetween(balanceString,"Errors per Minute Condition's value","was");
					
					if(StringUtils.isNotBlank(errPerMin))
						cnapMailNotificationObj.setErrorPerMin(errPerMin.trim());
				}
