...
@Component
public class ADBusTranRateHighParser implements WDNAPEmailParser {
	private static final Logger LOGGER = LoggerFactory
			.getLogger(ADBusTranRateHighParser.class);
	@Override
			...
			for (int i = ele.size() - 1; i > 0; i--) {
				...
				if (value.contains("Event Type")) {
					...
					if (ele.get(i + 1) != null && ele.get(i + 1).getElementsByTag("tr").size() >= 3) {
						...
						String innermsg = ADBusTranRateHighParser.parseInnerText(errorMessage, cnapMailNotificationObj);
