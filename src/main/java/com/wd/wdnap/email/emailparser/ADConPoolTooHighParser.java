...
@Component
public class ADConPoolTooHighParser implements WDNAPEmailParser {
	private static final Logger LOGGER = LoggerFactory
			.getLogger(ADConPoolTooHighParser.class);
	@Override
			...
			for (int i = ele.size() - 1; i > 0; i--) {
				...
				if (value.contains("Event Type")) {
					...
					if (ele.get(i + 1) != null && ele.get(i + 1).getElementsByTag("tr").size() >= 3) {
						...
						String innermsg = ADConPoolTooHighParser.parseInnerText(errorMessage, cnapMailNotificationObj);
