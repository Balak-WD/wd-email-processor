...
@Component
public class ADJvmGarTooHighParser implements WDNAPEmailParser {
	private static final Logger LOGGER = LoggerFactory
			.getLogger(ADJvmGarTooHighParser.class);
	@Override
			...
			for (int i = ele.size() - 1; i > 0; i--) {
				...
				if (value.contains("Event Type")) {
					...
					if (ele.get(i + 1) != null && ele.get(i + 1).getElementsByTag("tr").size() >= 3) {
						...
						String innermsg = ADJvmGarTooHighParser.parseInnerText(errorMessage, cnapMailNotificationObj);
	...
	private static String parseInnerText(String innerText, WDNAPMailNotificationObject cnapMailNotificationObj) {
		...
		if (StringUtils.isNoneEmpty(msg)) {
			if (msg.contains("JMX Instance Name")) {
				...
				if(StringUtils.isNotEmpty(jmxInstancevalue)){
			...
			if (msg.contains("1)") || msg.contains("condition 1") || msg.contains("JVM|Memory:")) {
				...
				if (balanceString.contains("threshold")) {
					...
					if(StringUtils.isNotEmpty(callsPerMin))
						cnapMailNotificationObj.setCallsPerMin(callsPerMin.trim());
					
					String gcTime=  StringUtils.substringBetween(balanceString, "GC Time Spent Per Min (ms)'s value","was");
						if(StringUtils.isNotEmpty(gcTime))
							cnapMailNotificationObj.setAvailVal(gcTime);
				}
