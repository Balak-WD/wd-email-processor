...
@Component
public class AppDynamicEmailParser implements WDNAPEmailParser {
	private static final Logger LOGGER = LoggerFactory
			.getLogger(AppDynamicEmailParser.class);
	@Override
			...
			for (int i = ele.size() - 1; i > 0; i--) {
				...
				if (value.contains("Event Type")) {
					...
					if (ele.get(i + 1) != null && ele.get(i + 1).getElementsByTag("tr").size() >= 3) {
						...
						String innermsg = AppDynamicEmailParser.parseInnerText(errorMessage, cnapMailNotificationObj);
